...
public class InvoiceController {
    ...
    private String validateInvoicePayload(Invoice invoice) {
        ...
        if (hasNegativeVatRate(invoice)) {
            return "vatRate cannot be negative";
        }

        for (int i = 0; i < invoice.getItems().size(); i++) {
            String error = validateInvoiceItem(
    ...
    private boolean hasNegativeVatRate(Invoice invoice) {
        return invoice.getVatRate() != null
                && invoice.getVatRate().compareTo(BigDecimal.ZERO) < 0;
    }

    private String validateInvoiceItem(InvoiceLineItem item, int lineNumber) {

        if (item.getType() == null) {
            return ITEM + lineNumber
                    + " must define type (SERVICE, MEDICATION, ADDITIONAL_CHARGE)";
        }
        ...
        if (item.getQuantity() == null || item.getQuantity() <= 0) {
            return ITEM + lineNumber
                    + " must include a quantity greater than zero";
        }

        if (item.getUnitPrice() == null
                || item.getUnitPrice().compareTo(BigDecimal.ZERO) < 0) {
            return ITEM + lineNumber
                    + " must include a unitPrice greater or equal to zero";
        }
